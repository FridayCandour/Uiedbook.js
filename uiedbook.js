...
export function lit(type, label) {
    if (!typer.log) {
...
export const animate = (name, ...properties) => {
    const styS = "@keyframes " + name + " " + "{", styE = "}", proplen = properties.length;
...
export const intersect = (target, opt, callback) => {
    const { root, rootMargin, threshold } = opt, options = {
        root: root,
...
export const buildCanvas = function (id, w = globalThis.innerWidth, h = globalThis.innerHeight) {
    const canv = document.createElement("canvas"), context = canv.getContext("2d"), backingStores = [
        "webkitBackingStorePixelRatio",
        "mozBackingStorePixelRatio",
        "msBackingStorePixelRatio",
        "oBackingStorePixelRatio",
        "backingStorePixelRatio"
    ], deviceRatio = globalThis.devicePixelRatio, backingRatio = backingStores.reduce(function (prev, curr) {
...
export class Entity {
    ...
    behaviors) {
        ...
        this.observeEntity = function (ent) {
            if (this.left > ent.left + ent.width ||
...
class imgPainter {
    ...
    update(entity) {
        ...
        if (this.range % this.delay === 0) {
            if (lit(this.observeChange, 422)({ left: entity.left, top: entity.top, width: entity.width, height: entity.height }))
                return;
...
const game = (function () {
    ...
    function below
    */
    let canvas, id, gameId, context, 
    /** the difference between these two is that one is used for calculating and the other is for the game rendering */
    fps, fpso = 0, lastdt = 0, pause = false, deltaTime, useBg = false, gameStart = true;
    const bg = [], entitysArray = [], 
    /**A canvas for the game */
    screen = buildCanvas("uiedbook_game_canvas"), painter = screen.getContext("2d");
