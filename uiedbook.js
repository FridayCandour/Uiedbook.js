...
export const game = (function () {
   ...
   sequenced on.*/
    ...
    function mount(template, callback) {
        gameframe = u("body").appendTo("div", { id: "gameframe" });
        if (games.length === 1) {
            return;
        }
        else {
...
export const spriteSheetPainter = function (img, horizontal = 1, vertical = 1, delay = 1) {
    ...
    export const renderer = (function () {
        let canvas, id, // for pausing or playing the game
        context, 
        // variables for the timing
        fps, fpso, 
        // background varible
        lastdt = 0, nextdt = 0, pause = false, deltaTime, started = false, useBg = false;
        const bg = [], 
        // entity storage array
        entitysArray = [], screen = buildCanvas("uiedbook_game_canvas"), painter = screen.getContext("2d"), pool = [];
